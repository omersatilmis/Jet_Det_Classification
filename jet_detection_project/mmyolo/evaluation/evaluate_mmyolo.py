"""
Jet Detection – MMYOLO Model Evaluator
=======================================
YOLOv8-S modelini val/test seti üzerinde değerlendirir.

Çıktılar:
  - COCO metrikleri (mAP, AP50, AP75, AR, per-class AP)
  - Confusion matrix (görsel + CSV)
  - Sınıf bazlı precision/recall tablosu
  - Hatalı tespit örnekleri (FP/FN)

Kullanım:
    python mmyolo/evaluation/evaluate_mmyolo.py
    python mmyolo/evaluation/evaluate_mmyolo.py --split test
    python mmyolo/evaluation/evaluate_mmyolo.py --score-thr 0.4
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="MMYOLO YOLOv8 Evaluator")
    p.add_argument("--split", choices=["val", "test"], default="val")
    p.add_argument("--config", type=str, default=None,
                   help="Config dosyası (varsayılan: mmyolo/configs/jet/yolov8_s_jet.py)")
    ...
    p.add_argument("--max-vis", type=int, default=20,
                   help="Hatalı tespit görselleştirme sayısı")
    p.add_argument("--out-dir", type=str, default=None,
                   help="Çıktı dizini (varsayılan: outputs/eval_mmyolo_<split>)")
    return p.parse_args()
...
def find_project_root() -> Path:
    ...
    for p in [here] + list(here.parents):
        if (p / "mmyolo").exists() and (p / "coco_annotations").exists():
            return p
...
def build_confusion_data(model, dataset, class_names, score_thr, iou_thr, device):
    """Her görsel için TP/FP/FN hesaplar."""
    ...
    for idx in range(len(dataset)):
        ...
        gt_boxes = [inst["bbox"] for inst in gt_instances]
...
def plot_confusion_matrix(confusion, class_names, out_path):
    ...
    im = ax.imshow(confusion, interpolation="nearest", cmap=plt.cm.Oranges)
    ax.set(xticks=range(len(labels)), yticks=range(len(labels)),
           xticklabels=labels, yticklabels=labels,
           ylabel="Predicted", xlabel="Ground Truth",
           title="Confusion Matrix (MMYOLO YOLOv8)")
...
def main():
    ...
    config_path = Path(args.config) if args.config else \
        project_root / "mmyolo" / "configs" / "jet" / "yolov8_s_jet.py"

    if args.work_dir:
        work_dir = Path(args.work_dir) if Path(args.work_dir).is_absolute() \
            else project_root / args.work_dir
    else:
        work_dir = project_root / "work_dirs" / "mmyolo" / "yolov8_s_jet"

        if not work_dir.exists():
            work_dir = project_root / "work_dirs" / "yolov8_s_jet"
    ...
    if ckpt_path is None or not ckpt_path.exists():
        ...
        print(f"  Önce modeli eğitin: python mmyolo/scripts/train_yolo_detection.py")
        sys.exit(1)
    ...
    # --- MMYOLO modüllerini kaydet ---
    from mmdet.utils import register_all_modules as register_det_modules
    register_det_modules()

    try:
        from mmyolo.utils import register_all_modules as register_yolo_modules
        register_yolo_modules()
    except ImportError:
        print("[WARN] mmyolo.utils.register_all_modules bulunamadı, "
              "mmyolo doğrudan import edilecek...")
    ...
    # Path yamalama — MMYOLO göreceli yollar kullanıyor, absolute'a çevir
    ...
    # Train dataloader — evaluation sırasında kullanılmaz ama config parse hatasını önler
    if hasattr(cfg, "train_dataloader"):
        tds = cfg.train_dataloader.dataset
        while hasattr(tds, "dataset"):
            tds = tds.dataset
        tds.data_root = ""
        tds.data_prefix = dict(img=str(images_dir) + os.sep)
        tds.ann_file = str(ann_dir / "instances_train.json")

    if hasattr(cfg, "val_evaluator"):
        cfg.val_evaluator.ann_file = str(ann_dir / ann_map["val"])
    if hasattr(cfg, "test_evaluator"):
        cfg.test_evaluator.ann_file = str(ann_dir / ann_map["test"])
    ...
    # ========== BÖLÜM 1: COCO Metrics ==========
    print("\n" + "=" * 70)
    print("BÖLÜM 1: COCO METRİKLERİ (MMYOLO YOLOv8)")
    print("=" * 70)
